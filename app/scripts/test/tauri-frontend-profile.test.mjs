...
const BROWSER_AUTOMATION_BACKEND_DESCRIPTOR = {
...
test('Tauri AI profile declares one exact deferred BrowserAutomation backend descriptor', async () => {
    const browserDirectory = path.join(appDirectory, 'applications', 'browser');
    const profile = JSON.parse(await fs.promises.readFile(path.join(browserDirectory, 'tauri-profile.json'), 'utf8'));

    assert.deepEqual(profile.featureGroups.ai.deferredBackendModules, [BROWSER_AUTOMATION_BACKEND_DESCRIPTOR]);
    const proxy = await fs.promises.readFile(path.join(browserDirectory, BROWSER_AUTOMATION_BACKEND_DESCRIPTOR.proxy), 'utf8');
    const feature = await fs.promises.readFile(path.join(browserDirectory, BROWSER_AUTOMATION_BACKEND_DESCRIPTOR.entry), 'utf8');
    assert.match(proxy, /ai-ide-browser-automation-feature\.cjs/);
    assert.doesNotMatch(proxy, /import\s*(?:\(|[^;]*from)\s*['"]@theia\/ai-ide\/lib\/node\/app-tester-agent\/browser-automation-impl/);
    assert.match(feature, /@theia\/ai-ide\/lib\/node\/app-tester-agent\/browser-automation-impl/);
    assert.match(feature, /createChild\(\)/);
});
...
test('emits and validates an exact deferred backend module contract', () => {
    const packages = {
        product: manifest('product'),
        '@theia/ai-ide': manifest('@theia/ai-ide'),
    };
    const featureGroups = deferredBackendModules => ({
        ai: {
            deferredRoots: [],
            blockedRoots: [{
                name: '@theia/ai-ide',
                reason: 'Only browser automation is deferred.',
            }],
            deferredBackendModules,
        },
    });
    const input = overrides => fixture({
        roots: ['product', '@theia/ai-ide'],
        packages,
        featureGroups: featureGroups([{ ...BROWSER_AUTOMATION_BACKEND_DESCRIPTOR, ...overrides }]),
    });

    const result = resolveProfile(input({}));
    assert.deepEqual(result.featureGroups.ai.deferredBackendModules, [BROWSER_AUTOMATION_BACKEND_DESCRIPTOR]);
    assert.ok(result.extensions.includes('@theia/ai-ide'));

    assert.throws(() => resolveProfile(input({ package: 'missing' })), /unknown deferred backend package "missing"/i);
    assert.throws(() => resolveProfile(input({ module: '../escape' })), /deferred backend.*canonical module request/i);
    assert.throws(() => resolveProfile(input({ proxy: '../escape.ts' })), /deferred backend module proxy.*canonical/i);
    assert.throws(() => resolveProfile(input({ output: '../escape.cjs' })), /deferred backend module output.*canonical/i);
    const duplicated = input({});
    duplicated.profileConfig.featureGroups.ai.deferredBackendModules.push(BROWSER_AUTOMATION_BACKEND_DESCRIPTOR);
    assert.throws(() => resolveProfile(duplicated), /deferred backend module.*duplicated/i);
});
...
test('backend build plans isolate the exact BrowserAutomation implementation and full profile remains unaliased', async t => {
    const deferredBuild = await import('../../applications/browser/tauri-src/backend/esbuild-backend-deferred.mjs');
    assert.equal(typeof deferredBuild.createTauriBackendBuildPlans, 'function');
    const directory = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'ride-backend-deferred-'));
    t.after(() => fs.promises.rm(directory, { recursive: true, force: true }));
    ...
    await Promise.all([
        fs.promises.copyFile(
            path.join(sourceDirectory, 'ai-ide-browser-automation-proxy.ts'),
            path.join(fixtureSourceDirectory, 'ai-ide-browser-automation-proxy.ts'),
        ),
        fs.promises.copyFile(
            path.join(sourceDirectory, 'ai-ide-browser-automation-feature.ts'),
            path.join(fixtureSourceDirectory, 'ai-ide-browser-automation-feature.ts'),
        ),
    ]);

    const writeModule = async (relativePath, source, packageManifest) => {
        const file = path.join(directory, 'node_modules', relativePath);
        await fs.promises.mkdir(path.dirname(file), { recursive: true });
        await fs.promises.writeFile(file, source);
        if (packageManifest) {
            await fs.promises.writeFile(
                path.join(directory, 'node_modules', packageManifest.path, 'package.json'),
                JSON.stringify({ name: packageManifest.name, version: '1.0.0', main: packageManifest.main ?? 'index.js' }),
            );
        }
    };
    await Promise.all([
        fs.promises.writeFile(path.join(directory, 'main.js'), "module.exports = require('@theia/ai-ide/lib/node/backend-module');\n"),
        writeModule(
            '@theia/ai-ide/lib/node/backend-module.js',
            "module.exports = require('./app-tester-agent/browser-automation-impl');\n",
            { path: '@theia/ai-ide', name: '@theia/ai-ide', main: 'lib/node/backend-module.js' },
        ),
        writeModule(
        ...
        writeModule(
            ...
            "exports.runtime = [require('@tootallnate/quickjs-emscripten'), require('chromium-bidi'), require('esprima')];\n",
            { path: 'puppeteer-core', name: 'puppeteer-core' },
        ),
        writeModule('@tootallnate/quickjs-emscripten/index.js', 'exports.quickjs = true;\n', {
            path: '@tootallnate/quickjs-emscripten', name: '@tootallnate/quickjs-emscripten',
        }),
        writeModule('chromium-bidi/index.js', 'exports.bidi = true;\n', {
            path: 'chromium-bidi', name: 'chromium-bidi',
        }),
        writeModule('esprima/index.js', 'exports.esprima = true;\n', {
            path: 'esprima', name: 'esprima',
        }),
    ]);
    ...
    const criticalManifest = {
        profile: 'tauri-critical',
        featureGroups: { ai: { deferredBackendModules: [BROWSER_AUTOMATION_BACKEND_DESCRIPTOR] } },
    };
    ...
    let resolver;
    aliasPlugin.setup({ onResolve(filter, callback) { resolver = { filter, callback }; } });
    const upstreamImporter = path.join(directory, 'node_modules', '@theia', 'ai-ide', 'lib', 'node', 'backend-module.js');
    assert.deepEqual(resolver.callback({
        path: './app-tester-agent/browser-automation-impl',
        importer: upstreamImporter,
        resolveDir: path.dirname(upstreamImporter),
    }), { path: path.join(directory, BROWSER_AUTOMATION_BACKEND_DESCRIPTOR.proxy) });
    assert.equal(resolver.callback({
        path: './app-tester-agent/browser-automation-impl',
        importer: path.join(directory, 'other.js'),
        resolveDir: directory,
    }), undefined);

    const [mainResult, featureResult] = await Promise.all([
        esbuild.build(plans.main),
        esbuild.build(plans.features[0].options),
    ]);
    const mainInputs = Object.keys(mainResult.metafile.inputs).map(input => input.replaceAll('\\', '/'));
    const featureInputs = Object.keys(featureResult.metafile.inputs).map(input => input.replaceAll('\\', '/'));
    const owns = (inputs, suffix) => inputs.some(input =>
        input === suffix.replace(/^\//, '') || input.endsWith(suffix),
    );
    assert.ok(owns(mainInputs, '/tauri-src/backend/ai-ide-browser-automation-proxy.ts'));
    assert.equal(owns(mainInputs, '/app-tester-agent/browser-automation-impl.js'), false);
    for (const runtime of ['/puppeteer-core/index.js', '/quickjs-emscripten/index.js', '/chromium-bidi/index.js', '/esprima/index.js']) {
        assert.equal(owns(mainInputs, runtime), false, `${runtime} leaked into backend main`);
        assert.ok(owns(featureInputs, runtime), `${runtime} missing from backend feature`);
    }
    assert.ok(owns(featureInputs, '/app-tester-agent/browser-automation-impl.js'));
    ...
    const fullResult = await esbuild.build({ ...fullPlans.main, outdir: path.join(directory, 'full') });
    const fullInputs = Object.keys(fullResult.metafile.inputs).map(input => input.replaceAll('\\', '/'));
    assert.ok(owns(fullInputs, '/app-tester-agent/browser-automation-impl.js'));
    assert.ok(owns(fullInputs, '/puppeteer-core/index.js'));
});

test('backend feature contexts are metadata-attested and disposed in watch and one-shot orchestration', async () => {
    ...
    assert.match(source, /backendFeatureContexts/);
    assert.match(source, /backendFeatureContexts\.map\(context\s*=>\s*context\.watch\(\)\)/);
    assert.match(source, /backendFeatureContexts\.map\(context\s*=>\s*context\.dispose\(\)\)/);
});
