...
function commandExists(command) {
  ...
  try {
    execSync(`${probe} ${command}`, { encoding: 'utf8', stdio: 'pipe' });
    return true;
  } catch (e) {
    return false;
  }
...
try {
  ...
  if (majorVersion >= 22) {
    ...
    logError(`Node.js version ${nodeVersion} is too old. Required: >= 22`);
  }
} catch (e) {
...
if (fs.existsSync(frontendDir)) {
  ...
  if (allFilesExist) {
    ...
    logWarning('Theia frontend build incomplete. Run from app/: yarn --cwd applications/browser build:prod');
  }
} else {
  logWarning('Theia frontend build not found. Run from app/: yarn --cwd applications/browser build:prod');
}
console.log();

// 5. 检查 Theia 后端构建
console.log('5. Checking Theia backend build...');
const backendDir = path.join(__dirname, '../browser/lib/backend');
if (fs.existsSync(backendDir) && fs.existsSync(path.join(backendDir, 'main.js'))) {
  logSuccess('Theia backend build found');
} else {
  logWarning('Theia backend build not found. Run from app/: yarn --cwd applications/browser build:prod');
}
console.log();
...
if (fs.existsSync(pluginsDir)) {
  ...
  if (pluginCount > 0) {
    ...
    logWarning('No plugins found. Run from app/: yarn download:plugins');
  }
} else {
...
if (fs.existsSync(tauriConfigPath)) {
  try {
    ...
    const missingIcons = configuredIcons.filter(icon => !fs.existsSync(path.join(__dirname, 'src-tauri', icon)));
    if (missingIcons.length === 0) {
      logSuccess(`Configured icons exist: ${configuredIcons.length}`);
    } else {
      for (const icon of missingIcons) {
        logError(`Configured icon missing: ${icon}`);
      }
    }
    ...
    } else if (fs.existsSync(path.join(__dirname, 'node_modules/.bin/tauri'))) {
      logSuccess('Tauri CLI available from local node_modules');
    } else if (fs.existsSync(workspaceTauriBin)) {
      logSuccess('Tauri CLI available from workspace node_modules');
    } else {
      logWarning('Tauri CLI not installed locally. Run yarn install from app/ before building.');
    }
  } catch (e) {
...
if (errors === 0 && warnings === 0) {
  ...
  console.log('1. cd app/applications/tauri');
