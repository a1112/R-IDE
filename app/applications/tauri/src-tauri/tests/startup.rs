...
use ride_tauri::startup::{
    attest_backend_process_session_snapshot, attest_pty_cleanup_session,
    begin_backend_priority_handoff, complete_backend_priority_handoff, finish_backend_stop,
    ...
    wait_for_owned_loopback, BackendLaunchPlan, BackendOwnershipState, BackendPriorityApi,
    BackendPriorityClass, BackendPriorityHandoff, BackendReadinessPolicy, BackendSpawnPlan,
    BackendSpawnStrategy, BackendStartupAction, BackendStartupEvent, BackendStartupState,
    BackendTransport, NoopBackendPriorityApi, RuntimePathMode, RuntimePaths, RuntimePathsCache,
    StartupWindowCreatedGate, StartupWindowCreatedOutcome, StartupWindowCreationGuard,
};
...
#[derive(Clone)]
struct FakeBackendPriorityApi {
    owned_identity: usize,
    calls: Arc<Mutex<Vec<(usize, BackendPriorityClass)>>>,
    fail_on: Option<BackendPriorityClass>,
    failure: Arc<str>,
}

impl BackendPriorityApi for FakeBackendPriorityApi {
    fn set_owned_root_priority(&self, priority: BackendPriorityClass) -> Result<(), String> {
        self.calls
            .lock()
            .expect("priority call mutex")
            .push((self.owned_identity, priority));
        if self.fail_on == Some(priority) {
...
fn fake_backend_priority_api(
    owned_identity: usize,
    calls: Arc<Mutex<Vec<(usize, BackendPriorityClass)>>>,
    fail_on: Option<BackendPriorityClass>,
    failure: impl Into<Arc<str>>,
) -> FakeBackendPriorityApi {
    FakeBackendPriorityApi {
        owned_identity,
        calls,
        fail_on,
...
#[tokio::test]
async fn backend_priority_targets_one_owned_root_until_the_window_gate_resolves() {
    let calls = Arc::new(Mutex::new(Vec::new()));
    let diagnostics = Arc::new(Mutex::new(Vec::new()));
    let handoff = Arc::new(BackendPriorityHandoff::new(fake_backend_priority_api(
        41,
        Arc::clone(&calls),
        None,
        "unused",
    )));
    assert!(begin_backend_priority_handoff(&handoff, {
        let diagnostics = Arc::clone(&diagnostics);
        move |diagnostic| diagnostics.lock().unwrap().push(diagnostic.to_string())
    }));
    assert_eq!(
        *calls.lock().unwrap(),
        vec![(41, BackendPriorityClass::BelowNormal)]
    );

    let gate = StartupWindowCreatedGate::default();
    let restore_gate = gate.clone();
    let restore_handoff = Arc::clone(&handoff);
    let restore_diagnostics = Arc::clone(&diagnostics);
    let restore = tokio::spawn(async move {
        let outcome = restore_gate.wait().await;
        complete_backend_priority_handoff(
            &restore_handoff,
            outcome == StartupWindowCreatedOutcome::Created,
            move |diagnostic| {
                restore_diagnostics
                    .lock()
                    .unwrap()
    ...
    gate.mark_created();
    assert!(restore.await.expect("priority restore task"));
    assert!(!complete_backend_priority_handoff(
        &handoff,
        true,
        |_| panic!("a completed handoff must not report twice"),
    ));
    assert_eq!(
        *calls.lock().unwrap(),
        vec![
            (41, BackendPriorityClass::BelowNormal),
            (41, BackendPriorityClass::Normal),
        ]
    );
...
#[test]
fn backend_priority_cleanup_cancels_restore_without_targeting_a_reused_pid() {
    let calls = Arc::new(Mutex::new(Vec::new()));
    let handoff = BackendPriorityHandoff::new(fake_backend_priority_api(
        41,
        Arc::clone(&calls),
        None,
        "unused",
    ));
    assert!(begin_backend_priority_handoff(&handoff, |_| {
        panic!("lowering the fake owned root must succeed")
    }));

    assert!(!complete_backend_priority_handoff(&handoff, false, |_| {
        panic!("cleanup cancellation is not a priority failure")
    }));
    assert!(!complete_backend_priority_handoff(&handoff, true, |_| {
        panic!("a cancelled handoff must stay inert")
    }));
    assert_eq!(
        *calls.lock().unwrap(),
        vec![(41, BackendPriorityClass::BelowNormal)],
        "the adapter retains the original process identity and never opens a replacement PID"
    );
}

#[test]
fn backend_priority_failures_are_single_bounded_diagnostics_only() {
    let long_failure = format!("priority failure\n{}", "x".repeat(1024));
    let lower_calls = Arc::new(Mutex::new(Vec::new()));
    let lower = BackendPriorityHandoff::new(fake_backend_priority_api(
        7,
        Arc::clone(&lower_calls),
        Some(BackendPriorityClass::BelowNormal),
        long_failure.clone(),
    ));
    let mut lower_diagnostics = Vec::new();
    assert!(!begin_backend_priority_handoff(&lower, |diagnostic| {
        lower_diagnostics.push(diagnostic.to_string())
    }));
    assert!(!complete_backend_priority_handoff(&lower, true, |_| {
        panic!("a failed lowering attempt must disable restoration")
    }));
    assert_eq!(lower_diagnostics.len(), 1);
    assert!(lower_diagnostics[0].chars().count() <= 256);
    assert!(!lower_diagnostics[0].contains(['\r', '\n']));

    let restore_calls = Arc::new(Mutex::new(Vec::new()));
    let restore = BackendPriorityHandoff::new(fake_backend_priority_api(
        9,
        Arc::clone(&restore_calls),
        Some(BackendPriorityClass::Normal),
        long_failure,
    ));
    assert!(begin_backend_priority_handoff(&restore, |_| {
        panic!("lowering must succeed before the injected restore failure")
    }));
    let mut restore_diagnostics = Vec::new();
    assert!(!complete_backend_priority_handoff(
        &restore,
        true,
        |diagnostic| restore_diagnostics.push(diagnostic.to_string()),
    ));
    assert!(!complete_backend_priority_handoff(&restore, true, |_| {
        panic!("restore failure must not retry")
    }));
    assert_eq!(restore_diagnostics.len(), 1);
    assert!(restore_diagnostics[0].chars().count() <= 256);
    assert!(!restore_diagnostics[0].contains(['\r', '\n']));
    assert_eq!(
        *restore_calls.lock().unwrap(),
        vec![
            (9, BackendPriorityClass::BelowNormal),
            (9, BackendPriorityClass::Normal),
        ]
    );
}

#[test]
fn non_windows_priority_adapter_is_an_explicit_no_op() {
    let handoff = BackendPriorityHandoff::new(NoopBackendPriorityApi);
    assert!(!begin_backend_priority_handoff(&handoff, |_| {
        panic!("the no-op adapter must not report a failure")
    }));
    assert!(!complete_backend_priority_handoff(&handoff, true, |_| {
        panic!("the no-op adapter must not attempt restoration")
    }));
}

#[tokio::test]
async fn window_creation_guard_resolves_created_or_cancelled_exactly_once() {
    let cancelled = StartupWindowCreatedGate::default();
    drop(StartupWindowCreationGuard::new(cancelled.clone()));
    assert_eq!(
        cancelled.wait().await,
        StartupWindowCreatedOutcome::Cancelled
    );

    let created = StartupWindowCreatedGate::default();
    let mut guard = StartupWindowCreationGuard::new(created.clone());
    guard.mark_created();
    drop(guard);
    assert_eq!(created.wait().await, StartupWindowCreatedOutcome::Created);
    created.cancel();
    assert_eq!(created.wait().await, StartupWindowCreatedOutcome::Created);
}

#[tokio::test]
async fn window_creation_gate_releases_registered_and_late_waiters() {
    let gate = StartupWindowCreatedGate::default();
    ...
    gate.mark_created();
    assert_eq!(
        tokio::time::timeout(Duration::from_secs(1), registered)
            .await
            .expect("registered window waiter must not lose its wakeup")
            .expect("registered window waiter task"),
        StartupWindowCreatedOutcome::Created
    );
    assert_eq!(
        tokio::time::timeout(Duration::from_secs(1), gate.wait())
            .await
            .expect("late window waiter must observe the stored outcome"),
        StartupWindowCreatedOutcome::Created
    );
}

#[test]
fn windows_priority_handoff_uses_the_retained_root_handle_after_job_assignment() {
    let startup_source = include_str!("../src/startup.rs");
    let claim = startup_source
        .split_once("AssignProcessToJobObject(self.job.0, root_process)")
        .expect("Windows Job assignment")
        .1
        .split_once("fn resume_suspended_process")
        .expect("suspended backend resume")
        .0;
    assert!(claim.contains("begin_backend_priority_handoff"));
    assert!(
        claim.find("begin_backend_priority_handoff").unwrap()
            < claim.find("resume_suspended_process").unwrap(),
        "the owned root is lowered only after Job assignment and before it can spawn descendants"
    );

    let windows_api = startup_source
        .split_once("impl BackendPriorityApi for WindowsBackendPriorityApi")
        .expect("Windows priority adapter")
        .1
        .split_once("impl PreparedPlatformBackendProcessTree")
        .expect("end of Windows priority adapter")
        .0;
    assert!(windows_api.contains("SetPriorityClass(self.root_process.0"));
    assert!(windows_api.contains("BELOW_NORMAL_PRIORITY_CLASS"));
    assert!(windows_api.contains("NORMAL_PRIORITY_CLASS"));
    assert!(!windows_api.contains("OpenProcess"));

    let tree_lifecycle = startup_source
        .split_once("impl BackendProcessTree {")
        .expect("backend process-tree implementation")
        .1
        .split_once("pub(crate) struct PreparedBackendProcessTree")
        .expect("end of backend process-tree implementation")
        .0;
    let regular_termination = tree_lifecycle
        .split_once("pub(crate) fn terminate_and_confirm")
        .expect("regular tree termination")
        .1
        .split_once("pub(crate) fn force_terminate_for_exit")
        .expect("forced tree termination")
        .0;
    assert!(regular_termination.contains("self.cancel_priority_handoff()"));
    let forced_termination = tree_lifecycle
        .split_once("pub(crate) fn force_terminate_for_exit")
        .expect("forced tree termination")
        .1
        .split_once("pub(crate) fn process_ids_bounded")
        .expect("end of forced tree termination")
        .0;
    assert!(forced_termination.contains("self.cancel_priority_handoff()"));

    let sidecar_source = include_str!("../src/sidecar.rs");
    let direct_start = sidecar_source
        .split_once("let backend_tree = match claim_direct_backend_tree")
        .expect("direct backend tree claim")
        .1
        .split_once("let stdout = match child.stdout.take()")
        .expect("direct backend stdout setup")
        .0;
    assert!(direct_start.contains("register_backend_tree"));
    assert!(direct_start.contains("schedule_backend_priority_restore"));
    assert!(
        direct_start.find("register_backend_tree").unwrap()
            < direct_start
                .find("schedule_backend_priority_restore")
                .unwrap()
    );
}
