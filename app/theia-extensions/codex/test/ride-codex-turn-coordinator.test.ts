...
async function settleTurnMicrotasks(rounds = 8): Promise<void> {
...
describe('RideCodexTurnCoordinator minimal streaming contract', () => {
    ...
    it('bounds a hung recovery acquire and releases its late lease exactly once', async () => {
        const host = new FakeTurnHost();
        const scheduler = new FakeScheduler();
        const timeoutCallbacks: Array<() => void> = [];
        const events: RideCodexUiEvent[] = [];
        ...
        assert.ok(timeoutCallbacks[1], 'recovery deadline remains active while acquire is pending');
        timeoutCallbacks[1]();
        assert.equal((await interrupting).status, 'interrupt-uncertain');
        while (scheduler.callbacks.length > 0) {
            scheduler.flushOne();
        ...
        host.nextTurnId = 'turn-2';
        assert.equal((await service.startTurn({
            threadId: 'thread-1', input: [{ type: 'text', text: 'after acquire deadline' }]
        })).turnId, 'turn-2');
        resolveAcquire({
            generation: 2,
            request: async () => validResumeResponse(),
    ...
    it('bounds a hung recovery resume, releases its lease, and emits one diagnostic', async () => {
        const host = new FakeTurnHost();
        const scheduler = new FakeScheduler();
        const timeoutCallbacks: Array<() => void> = [];
        const events: RideCodexUiEvent[] = [];
        ...
        host.interruptPromise = Promise.resolve({});
        host.resumeResponse = new Promise(() => undefined);
        ...
        assert.ok(timeoutCallbacks[1], 'resume shares the recovery deadline');
        assert.ok(resumeTimeout !== undefined && resumeTimeout > 0 && resumeTimeout <= 20);
        timeoutCallbacks[1]();
        assert.equal((await interrupting).status, 'interrupt-uncertain');
        while (scheduler.callbacks.length > 0) {
            scheduler.flushOne();
    ...
    it('uses one total recovery deadline across successful restart, acquire, and resume', async () => {
        const host = new FakeTurnHost();
        const timeoutCallbacks: Array<() => void> = [];
        const timeoutDurations: number[] = [];
        let clearedTimers = 0;
        ...
        resolveAcquire({
            generation: 2,
            request: async (_method, _params, timeoutMs) => {
                resumeTimeout = timeoutMs;
                return validResumeResponse();
            },
            release: () => { host.releases += 1; }
        });

        assert.equal((await interrupting).status, 'interrupt-uncertain');
        assert.deepEqual(timeoutDurations, [10, 20]);
        assert.equal(timeoutCallbacks.length, 2, 'recovery stages must not install independent timers');
        assert.equal(resumeTimeout, 7, 'resume receives only the remaining total recovery budget');
        assert.equal(clearedTimers, 2);
        await coordinator.dispose();
    });

    it('checks the total recovery deadline again after resume settles when its timer callback is delayed', async () => {
        for (const elapsed of [19, 20, 21]) {
            const host = new FakeTurnHost();
            const scheduler = new FakeScheduler();
            const timeoutCallbacks: Array<() => void> = [];
            const events: RideCodexUiEvent[] = [];
            ...
            const resumeResponse = new Promise<unknown>(resolve => { resolveResume = resolve; });
            const acquire = host.acquire.bind(host);
            host.interruptPromise = Promise.resolve({});
            host.resumeResponse = resumeResponse;
            host.acquire = async kind => {
                const lease = await acquire(kind);
                if (kind !== 'foreground-panel') {
                    return lease;
                ...
                return {
                    generation: lease.generation,
                    request: (method, params, timeoutMs) => lease.request(method, params, timeoutMs),
                    release: () => {
                        recoveryReleases += 1;
                        lease.release();
                    }
            ...
            assert.equal(host.calls.filter(call => call.method === 'thread/resume').length, 1);
            now = elapsed;
            resolveResume(validResumeResponse());
            assert.equal((await interrupting).status, 'interrupt-uncertain');
            while (scheduler.callbacks.length > 0) {
                scheduler.flushOne();
                await Promise.resolve();
            ...
            const expectedFailures = elapsed >= 20 ? 1 : 0;
            assert.equal(
                events.filter(event => event.type === 'error' && event.code === 'recovery-failed').length,
                expectedFailures,
                `elapsed=${elapsed} must ${expectedFailures === 1 ? '' : 'not '}fail recovery`
            );
            assert.equal(recoveryReleases, 1, `elapsed=${elapsed} releases the recovery lease once`);
            ...
            assert.equal(recoveryReleases, 1, 'a late timer callback must not release the lease twice');
            assert.equal(
                events.filter(event => event.type === 'error' && event.code === 'recovery-failed').length,
                expectedFailures,
            ...
            host.nextTurnId = 'turn-2';
            assert.equal((await service.startTurn({
                threadId: 'thread-1', input: [{ type: 'text', text: 'after settled recovery' }]
            })).turnId, 'turn-2');
            await coordinator.dispose();
    ...
    it('checks the recovery deadline after synchronous resume validation', async () => {
        for (const validationCompletedAt of [19, 20, 21]) {
            const host = new FakeTurnHost();
            const scheduler = new FakeScheduler();
            const timeoutCallbacks: Array<() => void> = [];
            const events: RideCodexUiEvent[] = [];
            ...
            const resumeResponse = validResumeResponse();
            const acquire = host.acquire.bind(host);
            const getOwnPropertyDescriptors = Object.getOwnPropertyDescriptors;
            host.interruptPromise = Promise.resolve({});
            host.resumeResponse = resumePromise;
            host.acquire = async kind => {
                const lease = await acquire(kind);
                if (kind !== 'foreground-panel') {
                    return lease;
            ...
            Object.getOwnPropertyDescriptors = (<T>(value: T) => {
                const descriptors = getOwnPropertyDescriptors(value);
                if (value === resumeResponse) {
                    now = validationCompletedAt;
                }
                return descriptors;
            ...
            try {
                ...
                const expectedFailures = validationCompletedAt >= 20 ? 1 : 0;
                assert.equal(
                    events.filter(event => event.type === 'error' && event.code === 'recovery-failed').length,
                    expectedFailures,
                    `validation completing at ${validationCompletedAt}ms must ${expectedFailures ? '' : 'not '}fail recovery`
                );
                assert.equal(recoveryReleases, 1, 'the recovery lease is released exactly once');
                assert.equal(clearedTimers, 2, 'interrupt and recovery timers are both cleared');
                ...
                assert.equal(recoveryReleases, 1, 'a late deadline callback must not release the lease twice');
                assert.equal(
                    events.filter(event => event.type === 'error' && event.code === 'recovery-failed').length,
                    expectedFailures,
                    'a late deadline callback must not duplicate diagnostics'
                );

                host.nextTurnId = 'turn-2';
                assert.equal((await service.startTurn({
                    threadId: 'thread-1', input: [{ type: 'text', text: 'after validation' }]
                })).turnId, 'turn-2');
            } finally {
    ...
    for (const snapshotValidatedAt of [19, 20, 21]) {
        it(`rechecks the recovery deadline after snapshot validation reaches ${snapshotValidatedAt}ms`, async () => {
            const host = new FakeTurnHost();
            const scheduler = new FakeScheduler();
            const timeoutCallbacks: Array<() => void> = [];
            const events: RideCodexUiEvent[] = [];
            ...
            let resolveRestart!: (generation: number) => void;
            const acquire = host.acquire.bind(host);
            const snapshot = host.snapshot.bind(host);
            host.interruptPromise = Promise.resolve({});
            host.restartPromise = new Promise(resolve => { resolveRestart = resolve; });
            host.snapshot = () => {
                snapshotCalls += 1;
                const current = snapshot();
                return snapshotCalls === 1 ? current : Object.freeze({
                    state: current.state,
                    get generation() {
                        now = snapshotValidatedAt;
            ...
            try {
                ...
                const expired = snapshotValidatedAt >= 20;
                assert.equal(snapshotCalls, 2);
                assert.equal(foregroundAcquireCalls, expired ? 0 : 1, 'expired recovery must not acquire');
                assert.equal(resumeCalls, expired ? 0 : 1, 'expired recovery must not resume');
                assert.equal(recoveryReleases, expired ? 0 : 1, 'only an acquired recovery lease is released');
                assert.equal(host.releases, expired ? 1 : 2, 'active and any recovery leases are cleaned up');
                assert.equal(clearedTimers, 2, 'interrupt and recovery timers are both cleared');
                assert.deepEqual(
                    events.filter(event => event.type === 'error' && event.code === 'recovery-failed'),
                    expired ? [{
                ...
                assert.equal(foregroundAcquireCalls, expired ? 0 : 1, 'late callback must not acquire');
                assert.equal(resumeCalls, expired ? 0 : 1, 'late callback must not resume');
                assert.equal(recoveryReleases, expired ? 0 : 1, 'late callback must not release twice');
                assert.equal(
                    events.filter(event => event.type === 'error' && event.code === 'recovery-failed').length,
                    expired ? 1 : 0,
                    'late callback must not duplicate diagnostics'
                );

                host.generation = expired ? 1 : 2;
                ...
                const starting = service.startTurn({
                    threadId: 'thread-2', input: [{ type: 'text', text: 'generation probe' }]
                });
                starting.catch(() => undefined);
                await settleTurnMicrotasks();
                host.emit('turn/started', { threadId: 'thread-2', turn: validTurn('turn-2') });
                while (scheduler.callbacks.length > 0) {
                    scheduler.flushOne();
                    await Promise.resolve();
                }
                assert.deepEqual(events, [{ type: 'turn-started' }],
                    'expired snapshot validation must not adopt the recovery generation');
                resolveStart({ turn: validTurn('turn-2') });
                assert.equal((await starting).turnId, 'turn-2');
            } finally {
                await coordinator.dispose();
            }
        });
